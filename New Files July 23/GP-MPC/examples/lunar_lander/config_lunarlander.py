...
def get_config(len_horizon=10, num_repeat_actions=5, include_time_model=False,):
	observation_config = ObservationConfig(
		obs_var_norm = [1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6]
	)

	reward_config = RewardConfig(
		target_state_norm=[0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 1.0, 1.0],

		weight_state=[1, 1, 1, 1, 1, 1, 1, 1], 
		weight_state_terminal=[5, 5, 5, 5, 5, 5, 5, 5], 

		target_action_norm=[0.5, 0.5], 
		weight_action=[0.05, 0.05],

		exploration_factor=1,

		use_constraints=False,
		state_min=[-2.5/10, -2.5/10, -1, -1, -6.2831855/10, -1, 0, 0], # ToDo
		state_max=[2.5/10, 2.5/10, 1, 1, 6.2831855/10, 1, 1/10, 0/10], # ToDo
		area_multiplier=1, 
	...
	model_config = ModelConfig(
		gp_init = {
            "noise_covar.noise": [1e-5, 1e-5, 1e-5, 1e-5, 1e-5, 1e-5, 1e-5, 1e-5],
            "base_kernel.lengthscale": [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
            "outputscale": [5e-2, 5e-2, 5e-2, 5e-2, 5e-2, 5e-2, 5e-2, 5e-2]
		},
		min_std_noise=1e-3,
		max_std_noise=1e-2,
	...
	memory_config = MemoryConfig(
		...
		min_error_prediction_state_for_memory=[3e-3, 3e-3, 3e-3, 3e-3, 3e-3, 3e-3, 3e-3, 3e-3],
		min_prediction_state_std_for_memory=[3e-3, 3e-3, 3e-3, 3e-3, 3e-3, 3e-3, 3e-3, 3e-3],
