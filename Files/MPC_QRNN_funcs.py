            ...
            # elif prob == "Acrobot": # Discrete actions
                ...
                if prob == "PandaReacher" or prob == "MuJoCoReacher" or prob == "PandaPusher" or prob == "MuJoCoPusher" or prob == "LunarLanderContinuous":
                    ...
                    # for j in range(horizon):
                        sim_states = torch.tensor(state, dtype=torch.float32).repeat(len(particles), 1)
                        # actions = torch.tensor(particles[:, j], dtype=torch.float32).reshape(len(particles),1)
                        actions_array = particles[:, j * action_dim : (j + 1) * action_dim]
                        actions = torch.tensor([actions_array], dtype=torch.float32).reshape(len(particles),action_dim)
                        ...
                        for loop_iter in range(num_particles):
                            particles[loop_iter, j * action_dim : (j + 1) * action_dim] = np.random.normal(action_mu.detach().numpy()[loop_iter], action_sigma.detach().numpy()[loop_iter])
        ...
        for step in range(prob_vars.max_steps):
            ...
            if not do_RS or not do_QRNN_step_rnd:
                if prob == "CartPole":
                    ...
                    new_values = np.random.uniform(action_low, action_high, size=(particles.shape[0], action_dim))
                    
                    # Add the new values to the last position
                    particles[:, -action_dim:] = new_values  
                    ...
                    # Generate new random values for the last column
                    new_values = np.random.uniform(action_low, action_high, size=(particles.shape[0], 1))
