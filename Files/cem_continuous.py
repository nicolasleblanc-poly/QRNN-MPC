...
class PlanningProblem(Problem):
    def __init__(self, prob_vars, state, model_QRNN, use_sampling, use_mid):
    ...
    @torch.no_grad()
    def predict_plan_outcome(self, state, state_dim, action_dim, plan_batch, model_QRNN, use_sampling, use_mid, action_low, action_high, states_low, states_high):
        ...
        for t in range(horizon):
            ...
            # costs += self.compute_cost(self.prob, state_batch, t, horizon, action_batch, self.goal_state)
...
def do_planning_cem(prob_vars, state, model_QRNN, use_sampling, use_mid):
    problem = PlanningProblem(prob_vars, state, model_QRNN, use_sampling, use_mid)
    searcher = CEM(
        problem,
        stdev_init = 0.5,
