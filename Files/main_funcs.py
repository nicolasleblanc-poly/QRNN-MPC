...
from MPC_QRNN_funcs import start_QRNN_MPC_wASGNN, start_QRNNrand_RS, start_QRNN_MPC_LBFGSB
from MPC_50NN_MSENN_funcs import start_50NN_MSENN_MPC_wASGNN, start_50NN_MSENNrand_RS, start_50NN_MSENN_MPC_LBFGSB
...
def main_50NN_MSENN_MPC(prob_vars, method_name, model_state, replay_buffer_state, optimizer_state, loss_state, model_ASN, replay_buffer_ASN, optimizer_ASN, do_RS, do_QRNN_step_rnd, use_sampling, use_mid, use_ASGNN):
    ...
    # for seed in tqdm(random_seeds):
        ...
        if not use_ASGNN:
            ...
            # print("episode_reward_list ", episode_reward_list, "\n")
...
def main_QRNN_MPC(prob_vars, method_name, model_QRNN, replay_buffer_QRNN, optimizer_QRNN, model_ASN, replay_buffer_ASN, optimizer_ASN, do_RS, do_QRNN_step_rnd, use_sampling, use_mid, use_ASGNN):
    ...
    # for seed in tqdm(random_seeds):
        ...
        if prob_vars.prob != "MuJoCoReacher" and prob_vars.prob != "MuJoCoPusher":
            if prob_vars.change_prob is None and prob_vars.std is not None:
                env = RecordVideo(prob_vars.env, video_folder="videos", episode_trigger=should_record, name_prefix=f"{prob_vars.prob}_{prob_vars.std}_{method_name}_seed{seed}") # , video_length=prob_vars.max_steps
            elif prob_vars.change_prob is not None and prob_vars.std is None:
                env = RecordVideo(prob_vars.env, video_folder="videos", episode_trigger=should_record, name_prefix=f"{prob_vars.prob}_{prob_vars.change_prob}_{method_name}_seed{seed}") # , video_length=prob_vars.max_steps
...
def main_50NN_MSENN_MPC_LBFGSB(prob_vars, method_name, model_state, replay_buffer_state, optimizer_state, loss_state, use_sampling, use_mid):
    # std is the std for continuous action spaces or the change prob
    # for discrete action spaces
    episode_rep_rewards = np.zeros((prob_vars.nb_rep_episodes, prob_vars.max_episodes))
    ...
    # for seed in tqdm(random_seeds):
        ...
        if prob_vars.prob == "PandaReacher" or prob_vars.prob == "PandaPusher" or prob_vars.prob == "MuJoCoReacher":
            episode_reward_list, episode_SuccessRate = start_50NN_MSENN_MPC_LBFGSB(prob_vars, env, seed, model_state, replay_buffer_state, optimizer_state, loss_state, use_sampling, use_mid)
        
            episode_rep_SuccessRate[rep_ep] = np.array(episode_SuccessRate)
        else:
            episode_reward_list = start_50NN_MSENN_MPC_LBFGSB(prob_vars, env, seed, model_state, replay_buffer_state, optimizer_state, loss_state, use_sampling, use_mid)
...
def main_QRNN_MPC_LBFGSB(prob_vars, method_name, model_QRNN, replay_buffer_QRNN, optimizer_QRNN, use_sampling, use_mid):
    # std is the std for continuous action spaces or the change prob
    # for discrete action spaces
    episode_rep_rewards = np.zeros((prob_vars.nb_rep_episodes, prob_vars.max_episodes))
    ...
    # for seed in tqdm(random_seeds):
        ...
        if prob_vars.prob == "PandaReacher" or prob_vars.prob == "PandaPusher" or prob_vars.prob == "MuJoCoReacher" or prob_vars.prob == "PandaReacherDense":
            episode_reward_list, episode_SuccessRate = start_QRNN_MPC_LBFGSB(prob_vars, env, seed, model_QRNN, replay_buffer_QRNN, optimizer_QRNN, use_sampling, use_mid)
        
            episode_rep_SuccessRate[rep_ep] = np.array(episode_SuccessRate)
        else:
            episode_reward_list = start_QRNN_MPC_LBFGSB(prob_vars, env, seed, model_QRNN, replay_buffer_QRNN, optimizer_QRNN, use_sampling, use_mid)
