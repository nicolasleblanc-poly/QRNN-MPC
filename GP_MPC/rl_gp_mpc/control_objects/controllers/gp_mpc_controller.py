...
class GpMpcController(BaseControllerObject):
    ...
    def get_action(self, obs_mu:np.array, obs_var:np.array=None, random:bool=False):
        ...
        if (self.iter_ctrl % self.config.controller.num_repeat_actions) == 0:
            ...
            # print("actions_raw.shape ", actions_raw.shape, "\n")
            # if action_dim > 1:
            #     next_action_raw = actions_raw[:action_dim]
            # else:
        ...
        # print("np.array(next_action_raw) ", np.array(next_action_raw), "\n")
    ...
    def check_and_close_processes(self):
        ...
        if 'p_train' in self.__dict__ and not self.p_train._closed and not (self.p_train.is_alive()):
            ...
            #     print("\n=== DEBUGGING GP INITIALIZATION ===")
            #     print(f"Model index: {model_idx}")
            #     print("Params to initialize:")
            #     for key, val in params_dict_list[model_idx].items():
            #         print(f"  {key}: {val}")

            #     # Also, print constraints if possible:
            #     print("Model constraints:")
            #     model = self.transition_model.models[model_idx]
            #     if hasattr(model, 'named_parameters'):
            #         for name, param in model.named_parameters():
            #             print(f"  Param {name}: {param.data}")

            #     if hasattr(model, 'named_constraints'):
            #         for name, constraint in model.named_constraints():
            #             print(f"  Constraint on {name}: {constraint}")
                
                
            #     model_params = params_dict_list[model_idx]

            #     # Fix outputscale if it's slightly below the lower bound (1e-2)
            #     if 'covar_module.outputscale' in model_params:
            #         outputscale = model_params['covar_module.outputscale']
            #         if outputscale < 1e-2:
            #             print(f"⚠️ Outputscale {outputscale} too small. Clipping to 1e-2.")
            #             model_params['covar_module.outputscale'] = 1e-2

            #     # Fix likelihood.noise if it's zero or below lower bound
            #     if 'likelihood.noise' in model_params:
            #         noise = model_params['likelihood.noise']
            #         if isinstance(noise, list) and noise[0] < 1e-6:
            #             print(f"⚠️ Likelihood noise {noise} too small. Clipping to 1e-6.")
            #             model_params['likelihood.noise'] = [1e-6]
            #         elif isinstance(noise, float) and noise < 1e-6:
            #             print(f"⚠️ Likelihood noise {noise} too small. Clipping to 1e-6.")
            #             model_params['likelihood.noise'] = 1e-6

            #     self.transition_model.models[model_idx].initialize(**model_params)
    ...
    #         self.p_train.join()
    #         for model_idx in range(len(self.transition_model.models)):
    #             # Clamp unsafe parameters before calling initialize
    #             for key, value in params_dict_list[model_idx].items():
    #                 if isinstance(value, torch.Tensor) and torch.any(torch.isnan(value)):
    #                     print(f"[Warning] NaN found in {key}, replacing with small constant.")
    #                     params_dict_list[model_idx][key] = torch.full_like(value, 1e-4)
    #                 elif key == "lengthscale":
    #                     params_dict_list[model_idx][key] = torch.clamp(value, min=1e-5)
    #                 elif key == "noise":
    #                     params_dict_list[model_idx][key] = torch.clamp(value, min=1e-4)
    ...
    def compute_mean_lcb_trajectory(self, actions_mpc:np.array, obs_mu:torch.Tensor, obs_var:torch.Tensor):
        ...
        # print("states_mu_pred.shape ", states_mu_pred.shape,"\n")
        # print("states_var_pred.shape ", states_var_pred.shape,"\n")
        # print("actions_model.shape ", actions_model.shape,"\n")
        ...
        # print("rewards_traj_ucb.shape ", rewards_traj_ucb.shape,"\n")
        # print("rewards_traj_ucb ", rewards_traj_ucb,"\n")
        ...
        # print("mean_reward_traj_ucb ", mean_reward_traj_ucb, "\n")
